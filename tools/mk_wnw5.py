...
EDGES = [-0.047673, -0.022461, -0.001398, 0.024163]     # w_success 5분위
NEDGES = [161.0, 387.0, 687.0, 1217.0]                   # w_n 5분위 (eda54 후속)
MODE = os.environ.get('TW_MODE', 'team')                 # team | wn
...
if MODE == 'wn':
    # 왼쪽 축을 pitcher_team 대신 w_n 구간으로 (수치 x 수치, 25칸)
    LEFT_TR = "np.digitize(df['w_n'].to_numpy(dtype='float64'), %r).astype(int).astype(str)" % NEDGES
    LEFT_IN = 'np.digitize(df_proc["w_n"].to_numpy(dtype="float64"), %r).astype(int).astype(str)' % NEDGES
    COL = 'wnw5'
else:
    LEFT_TR = "df['pitcher_team_id'].astype(str)"
    LEFT_IN = 'df_proc["pitcher_team_id"].astype(str)'
    COL = '%s' % COL
...
N1 = A1 + """
    ...
    df['%s'] = (%s + '|'
                  + """ % (COL, LEFT_TR) + BIN.replace('{V}', "df['w_success'].to_numpy(dtype='float64')") + """)"""
sub(find(A1)[0], A1, N1)
...
N2 = A2 + """
        ...
        df_proc["%s"] = (%s + "|"
                           + """ % (COL, LEFT_IN) + BIN.replace('{V}', 'df_proc["w_success"].to_numpy(dtype="float64")') + """)"""
sub(find(A2)[0], A2, N2)
...
for i in code:
    c = src(i).count(CAT)
    if c:
        sub(i, CAT, "'is_full_count', 'count_advantage', 'phteam', '" + COL + "', 'is_waste_pitch_sit',", n=c)
        n += c
if n == 0:
...
assert "%s" % COL in X_full.columns, "tmw5 누락"
assert "%s" % COL in cat_features, "tmw5 가 범주형으로 등록되지 않았다"
_v = X_full["%s" % COL].astype(str)
