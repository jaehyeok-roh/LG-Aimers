...
### ⚠️⚠️ 앙상블 규모가 결론의 부호를 바꾼다 (2026-08-20) — 지금까지 최대의 방법론 오류

**증상**: 반복수 1000 → 500 이 홀드아웃에서 +30 이었는데 리더보드에서 **-23** 이었다.
게다가 리더보드는 트리가 **많을수록** 좋다고 말한다 (500: 967.89 / 700: 986.21 / 1000: 990.95).

**원인**: 홀드아웃은 모델 3개 평균, 실운영은 **30개** 평균이다.
후반 반복의 과대적합은 대부분 **분산** 성분(모델마다 *다른* 노이즈를 외움)이라 평균으로
상쇄된다. 남는 **편향** 성분(모델들이 *같은* 헛된 패턴을 학습)은 평균으로 안 없어진다.
1000회 지점의 손해는 거의 전부 분산이었으므로 30개를 평균하면 사라진다.

**우리 데이터가 그대로 보여준다** (절대 수치는 척도가 달라 형태만 볼 것):

| 앙상블 규모 | 500 → 700 | 700 → 1000 | 형태 |
|---|---|---|---|
| 1개 (`aimers-loss`) | -8 | -22 | 하락 |
| 3개 (v7 오프셋 셀) | +6 | — | 평평 |
| **30개 (리더보드)** | **+18.3** | **+4.7** | **상승** |

앙상블이 커질수록 하락이 사라지고 상승으로 돌아선다. 세 점이 순서대로 찍혔다.

**규칙**:
1. 검증은 **실운영과 같은 앙상블 규모**로 할 것. 못 하면 최소한 *"이 결론이 모델 개수에
   의존하는가"* 를 먼저 물을 것. **반복수·정규화·깊이·용량 관련 결론은 전부 의존한다.**
   피처 추가/제거는 상대적으로 덜 의존한다.
2. **지금까지의 스크리닝은 전부 300회 / depth 6 / 모델 1~3개였다.** 그 조건은 체계적으로
   "용량을 줄여라"는 답을 낸다. 그 조건에서 기각한 것들은 30개 앙상블에서 다를 수 있다.
3. 반복수 축은 1000 근처에서 **이미 포화**했다(기울기가 4분의 1로 꺾임). 1200~1500 은
   +1~3 또는 마이너스로 추정되므로 제출을 태울 가치가 없다. **용량을 더 주려면 깊이 축**
   (v3 에서 Optuna 가 depth 9 를 골랐고 v4 에서 8 로 내린 뒤 그대로다).
...
| 8/21~8/27 | **모델 다양화만 남았다.** LightGBM 블렌딩 → NN 엔티티 임베딩 순. 블렌드 가중치는 오프셋 셀(±13 노이즈) 말고 **같은 fold 구조의 OOF** 로 고를 것 — 두 모델을 같은 fold 에서 뽑아 비교하는 것이라 시즌 의존성이 낮다. 여유 시 **`cond_*` 시즌 감쇠** |
