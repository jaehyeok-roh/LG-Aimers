...
| 8/21~8/27 | **모델 다양화가 최우선** (LightGBM/NN 임베딩 블렌딩). 1등 1,243 은 우리보다 설명분산이 56% 많은데, 단일 모델 계열의 피처 추가로는 메울 수 없는 폭이다. 그 다음 **seed 3→10**(무위험, 분산 감소), **depth 8→9~10**(앙상블이 용량을 더 감당한다는 v7 의 함의), **`cond_*` 시즌 감쇠** |
