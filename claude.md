...
| `aimers_v7.ipynb` | 700회 학습 + `ntree_end` 350/500/700 세 벌 제출 | ✅ 커밋됨 | 986.21 / 967.89 → 기각 |
| `aimers_loss.ipynb` | 손실함수 × 반복수 곡선 (2024 홀드아웃) | ✅ 커밋됨 | — |
| `aimers_topt.ipynb` | 2023 홀드아웃 목적함수 Optuna (실패) | ✅ 커밋됨 | — |
| `aimers_baseline.ipynb` | 피처 구성 A/B/C 비교 v1 (반복수 3000 으로 무효) | ✅ 커밋됨 | — |
| `aimers_converge.ipynb` | 반복수 수렴 진단 (모델 1개 — 결론이 뒤집힌 원인) | ✅ 커밋됨 | — |

`aimers_v7.ipynb` 는 기각됐지만 **지점별 isotonic + 다중 zip 산출 구조**는 재사용 가치가
있다. 학습 한 판으로 제출본 여러 개를 뽑아야 할 때 이 노트북의 Cell 6b/6c/7 을 참고할 것.
...
| 8/22~8/28 | **NN 재설계(v3)가 유일하게 두 자릿수를 넘길 후보다.** 용량을 확 줄이고 정규화를 세게 건다. 판정: 단독 600 이상 + 상관 0.85 미만 + 블렌드 +20 이상. 병행: **타자측 트랙맨(step19~21) 독립 재검증** — `zone_speed` 와 묶여 기각됐으나 범인은 `zone_speed` 였고 타자 피처 단독으로는 검증된 적이 없다 |
