...
| 8/22~8/28 | **NN 엔티티 임베딩**이 사실상 마지막 카드다. 투수/타자를 벡터로 학습해 트리와 다르게 틀리는 모델을 만든다. 판정 기준: cb_sym 과 예측 상관 0.85 미만 + 블렌드 +20 이상. 병행: **타자측 트랙맨(step19~21) 독립 재검증** — `zone_speed` 와 묶여 기각됐으나 범인은 `zone_speed` 였음이 밝혀졌고 타자 피처 단독으로는 검증된 적이 없다 |
