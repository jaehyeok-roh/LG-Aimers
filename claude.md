...
**현재 상태**: 리더보드 최고점 **987.3936** (`aimers_tuned_ensemble.ipynb` v4, 2026-08-19 확정).
959.43 대비 +27.97점 (조건부 투수통계 + 재중심화 + 죽은 피처 제거).
목표: 최소 1100점. **마감 9/2.**
...
### 현재 채택 구성 (987.3936점, `aimers_tuned_ensemble.ipynb` v4)
...
| 시도 | 결과 |
|---|---|
| **v4: 조건부 투수통계 + 재중심화 + 죽은 피처 제거** | **987.3936 (959.43 대비 +27.97점) → 채택, 현재 최고점** |
| Optuna 튜닝 + fold10 + seed3 앙상블 | 959.4258 (932.96 대비 +26.47점) → v4의 기반 |
| 트랙맨 매핑 재구축 (v2) | 홀드아웃 +9(±2). 아직 리더보드 미검증 — v5 후보 |
...
| 노트북 | 내용 | 리포 포함 여부 | 점수 |
|---|---|---|---|
| `aimers_tuned_ensemble.ipynb` (현재 최고) | v4 = Optuna + fold10 + seed3 + 조건부통계 + 재중심화 | ✅ 커밋됨 | **987.3936 (현재 채택)** |
...
| 8/19 | ✅ v4(조건부통계+재중심화+죽은피처제거) — **987.3936로 채택** / ✅ 트랙맨 매핑 재구축 |
| 8/20~8/27 | v5 후보 3장: **트랙맨 v2 매핑(+9±2)**, **RMSE 손실(+15±6, 실제 용량에서 재확인 필요)**, `asof_*` 재설계(최근 N구 이동평균) |
